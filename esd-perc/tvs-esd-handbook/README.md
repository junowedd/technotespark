# TVS / ESD Protection Engineering Handbook

A structured engineering reference covering:

- Parasitic inductance fundamentals  
- Transient voltage overshoot  
- Snapback physics  
- Parasitic BJT turn-on  
- SCR regenerative conduction  
- Inductance interaction with snapback  
- Stability and latch-up  
- Practical design strategies  

---

## Table of Contents

1. [Fundamentals](./docs/01_fundamentals.md)
2. [Inductance & Transients](./docs/02_inductance_and_transients.md)
3. [Snapback Physics](./docs/03_snapback_physics.md)
4. [Parasitic BJT Structure](./docs/04_BJT_structure.md)
5. [SCR Turn-On Structure](./docs/05_SCR_structure.md)
6. [Inductance vs Snapback Interaction](./docs/06_inductance_vs_snapback.md)
7. [Snapback Stability & Latch-Up](./docs/07_stability_and_latchup.md)
8. [Design Guidelines](./docs/08_design_guidelines.md)

---

## Scope

This handbook focuses on the physical and circuit-level mechanisms governing:

- TVS dynamic behavior  
- Snapback triggering  
- Parasitic inductance effects  
- ESD robustness engineering  
